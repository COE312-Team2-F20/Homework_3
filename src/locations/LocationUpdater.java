...
public class LocationUpdater extends Location {
	...
	public boolean newRoom = false;
	public boolean defeat = false;
	// Scanner s = new Scanner(System.in);
	Reader reader;

	public LocationUpdater(Reader reader) {
		this.reader = reader;
		location = "";
	}
	...
	public void updater() {
		...
		while (!getLocation().contains("mischief managed") && !found && !exit) {
			if (getLocation().contains("north")) {
				...
				next = "";
				PotionsClass student_p = new PotionsClass(reader);
				student_p.greeting();
				setRoom(true);
				student_p.look();
				if(getdefeat())
	...
	public synchronized boolean newRoom() {
		return newRoom;
	}
	public synchronized void setRoom(boolean flag) {
		newRoom = flag;
	}
	public synchronized boolean getdefeat() {
		return defeat;
	}
	public synchronized void setdefeat(boolean flag) {
		defeat = flag;
	}
